...
async function addStaffDentistIds() {
  ...
  try {
    ...
    console.log('Adding staff_id and dentist_id columns to users table...\n');

    // Add staff_id column (for Staff role users)
    await client.query(
      `ALTER TABLE users ADD COLUMN staff_id SERIAL UNIQUE`
    );
    console.log('✓ Added staff_id column');

    // Add dentist_id column (for Admin role users)
    await client.query(
      `ALTER TABLE users ADD COLUMN dentist_id SERIAL UNIQUE`
    );
