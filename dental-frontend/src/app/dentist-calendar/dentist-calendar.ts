...
export class DentistCalendarComponent implements OnInit {
  ...
  private loadAllUpcoming(): void {
    ...
    this.api.getDentistAppointments(this.dentistName).subscribe({
      next: (data) => {
        ...
        this.allUpcoming = data.filter(a =>
          !a.status.startsWith('Cancelled') && a.status !== 'No-show' && a.appointment_date >= today
        );
  ...
  get monthDays(): any[] {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      const events = this.allUpcoming.filter(a => a.appointment_date === dateStr && !a.status.startsWith('Cancelled'));
